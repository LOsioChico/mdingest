...
| Agent discovery (`.well-known/`) | Runtime-verified | `curl /.well-known/mcp/server-card.json` → 200 `application/json` (MCP Server Card with streamable-http transport); `curl /.well-known/ai-catalog.json` → 200 `application/json` (AI Catalog with 1 MCP entry); isitagentready.com scan: Agent-Readable L3 — robots.txt pass, sitemap pass, Link headers pass, markdown negotiation pass, content signals pass, AI bot rules pass |
