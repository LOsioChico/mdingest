...
| LLM visibility (`src/common/llm-visibility.ts`) | Runtime-verified | `curl /index.md` → 200 `text/markdown; charset=utf-8` + `Link: </>; rel="alternate"; type="text/html"`; `curl -H "Accept: text/markdown" /` → markdown (content negotiation); `curl -H "Accept: text/html" /` → HTML; q-value test passes both directions; `Accept: */*` → HTML (no false flip); `Link` + `Vary: Accept` on all 3 pages; 406 for `Accept: application/json` on web pages only; `.well-known/` paths skip negotiation (404 not 406); `robots.txt` + `llms.txt` + `llms-full.txt` + `sitemap-index.xml` served; `<link rel="alternate">` in HTML head; hidden markdown pointer in DOM; freshness date in footer; 7 FAQ items on landing page; `bun run verify` 0 errors; 118 tests pass; `build:web` succeeds with sitemap |
| Agent discovery (`.well-known/`) | Runtime-verified | `curl /.well-known/mcp/server-card.json` → 200 `application/json` (MCP Server Card with serverInfo.name + transport + capabilities); `curl /.well-known/ai-catalog.json` → 200 `application/json` (AI Catalog with 1 MCP entry); `curl /.well-known/api-catalog` → 200 `application/linkset+json` (RFC 9727 linkset with service-desc, sitemap, describedby, alternate rels); `Link: </.well-known/api-catalog>; rel="api-catalog"` header on all HTML pages; isitagentready.com scan: Agent-Readable L3 — robots.txt pass, sitemap pass, Link headers pass, markdown negotiation pass, content signals pass, AI bot rules pass, ARD pass |
