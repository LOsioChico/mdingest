...
import { DevtoInvalidUrlError } from "./errors/devto-errors.ts";
...
function makeResult(): ConvertResult {
  return {
    metadata: {
      title: "Test Article",
      author: "Jane Doe",
      source_url: "https://dev.to/testuser/test-article",
      provider: "devto",
      tags: ["python"],
      free: true,
    },
...
function makeController(result: ConvertResult = makeResult()): {
  controller: DevtoController;
  convertMock: ReturnType<typeof vi.fn>;
} {
  const convertMock = vi.fn().mockResolvedValue(result);
  const controller = new DevtoController({ convert: convertMock } as never);
...
describe("DevtoController", () => {
  describe("GET /v1/devto (format=markdown, default)", () => {
    ...
    it("calls devtoService.convert with the URL", async () => {
      ...
      expect(convertMock).toHaveBeenCalledWith("https://dev.to/user/article");
    });
  });

  describe("GET /v1/devto (format=json)", () => {
    it("returns object with metadata and markdown", async () => {
      const { controller } = makeController();
      const { reply } = makeReply();

      const result = await controller.convert(
        { url: "https://dev.to/user/article", format: "json" },
        reply as never,
      );
  ...
  describe("error propagation", () => {
    it("propagates DevtoInvalidUrlError from service", async () => {
      const convertMock = vi.fn().mockRejectedValue(new DevtoInvalidUrlError("https://example.com"));
      const controller = new DevtoController({ convert: convertMock } as never);
      const { reply } = makeReply();
      ...
      ).rejects.toThrow(DevtoInvalidUrlError);
    });

    it("propagates generic errors from service", async () => {
      const convertMock = vi.fn().mockRejectedValue(new Error("Dev.to API down"));
      const controller = new DevtoController({ convert: convertMock } as never);
      const { reply } = makeReply();
      ...
      ).rejects.toThrow("Dev.to API down");
    });
  });
});
